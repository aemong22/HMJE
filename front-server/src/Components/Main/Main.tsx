...
function Main(): JSX.Element {
  ...
  const {data:levelRank, error:error6, isLoading:isLoading6} = useGetUserRankLevelQuery('');
  const {data:wordRank, error:error7, isLoading:isLoading7} = useGetUserRankWordQuery('');
  ...
  return (
    <>
      ...
      <Ranking levelInfo={levelInfo} levelRank={levelRank?.data} wordRank={wordRank?.data} userMyInfo={userMyInfo?.data} userMyStudy={userMyStudy?.data}/>
      <Footer />
    </>
  );
}
      ...
      <div className="container max-w-screen-xl xl:w-[80%] w-full mx-auto px-[5%] md:my-[7rem] my-[5rem] md:px-10">
        ...
        <div className="">
          <div className="md:text-[2.5rem] text-[2.1rem] font-bold">
              <span className="md:text-[2rem] text-[1.5rem]">오늘의 </span><span className="text-[#BE8D65]">신문</span> 핵심단어
          </div>
          <div className="flex mt-4">
            <div className={classNames("text-[1.4rem] px-4 py-1 font-bold",select === 0 ? "border-b-4 border-[#F7CCB7]" : null)}
              onClick={()=>{
                firstSelect(0)
                setSelect(0)
              }}>
              <div className={`${style.menu}`}  style={{backgroundImage:`url('/Assets/Icon/${words[0]?.category}.png')`}}></div>
              {words[0]?.category}
            </div>

            <div className={classNames("text-[1.4rem] px-4 py-1 font-bold",select === 1 ? "border-b-4 border-[#F7CCB7]" : null)}
              onClick={()=>{
                firstSelect(1)
                setSelect(1)
            }}>
              <div className={`${style.menu}`}  style={{backgroundImage:`url('/Assets/Icon/${words[1]?.category}.png')`}}></div>
              {words[1]?.category}
            </div>

            <div className={classNames("text-[1.4rem] px-4 py-1 font-bold",select === 2 ? "border-b-4 border-[#F7CCB7]" : null)}
              onClick={()=>{
                firstSelect(2)
                setSelect(2)
              }}>
              <div className={`${style.menu}`}  style={{backgroundImage:`url('/Assets/Icon/${words[2]?.category}.png')`}}></div>
              {words[2]?.category}
            </div>

            <div className={classNames("text-[1.4rem] px-4 py-1 font-bold",select === 3 ? "border-b-4 border-[#F7CCB7]" : null)}
              onClick={()=>{
                firstSelect(3)
                setSelect(3)
              }}>
              <div className={`${style.menu}`}  style={{backgroundImage:`url('/Assets/Icon/${words[3]?.category}.png')`}}></div>
              {words[3]?.category}
            </div>
        ...
        <div className="md:text-[1.8rem] sm:text-[1.4rem] text-[1.2rem] font-bold">
          ...
          <span className="md:text-[1rem] sm:text-[0.9rem] text-[0.8rem] font-medium text-[#A2A2A2]">
            ...
            {selectWord?.detail?.wordOrigin && <>[{selectWord?.detail?.wordOrigin}]</>} {" "}
